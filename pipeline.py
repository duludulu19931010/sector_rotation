...
def load_groups() -> dict[str, list[str]]:
    ...
    for col_idx in range(0, n_cols, 2):
        ...
        for row_idx in range(2, len(df)):
            ...
            if val and val != 'nan':
                codes.append(val.zfill(4))
        if codes:
            groups[gname] = codes
    log.info(f"Groups: {len(groups)}, {sum(len(v) for v in groups.values())} stocks")
    return groups
    ...
    if not args.dry_run:
        ...
        if len(existing_dates) < 21 or args.reset_history:
            ...
            if not hist_df.empty:
                ...
                if not hist_df.empty:
                    hist_df["name"] = hist_df["code"].map(name_map).fillna("")
                    db_save(hist_df)
