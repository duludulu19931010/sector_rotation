...
def step2_institutional(force: bool = False) -> pd.DataFrame:
    ...
    # 去 API 抓今日 + 補缺漏的日期（cached_dates 的日期自動跳過）
    ...
    if not df.empty:
        ...
        if not new_df.empty:
            ...
            log.info(f"  Saved {len(new_df)} new rows to DB")

    # 無論 API 結果如何，從 DB 回傳最新 20 日資料
    # 若今日 API 還沒資料（收盤前），會用昨日最新的 20 日計算，這是正確行為
    result = db.load_institutional(days=20)
    if result.empty and not cached_dates:
        log.warning("  API returned nothing and DB empty — pipeline cannot proceed")
    elif result.empty:
        log.warning("  DB load returned empty despite cached_dates existing")
    else:
        log.info(f"  Using {result['trade_date'].nunique()} trading days from DB")
    return result
...
def step3_prices(force: bool = False) -> pd.DataFrame:
    ...
    if not df.empty:
        ...
        log.info(f"  Saved {len(df)} prices for {TODAY}")
        return df

    # 若 API 失敗（如今日非交易日或收盤前），用 DB 最新一日
    fallback = db.load_prices()
    if not fallback.empty:
        latest = fallback["trade_date"].max() if "trade_date" in fallback.columns else "unknown"
        log.warning(f"  API failed, using DB latest prices ({latest})")
    else:
        log.error("  No price data available from API or DB")
    return fallback
